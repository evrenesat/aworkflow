...
5. First launch asks for a password. The password is also configurable in global `aflow.toml`.
6. The projects root is a global directory setting in that same file.
...
Use `~/.config/aflow/aflow.toml` and sibling `workflows.toml`. Add a typed `[ui]` table to `aflow.toml`:
...
port = 8765
projects_root = "~/code"
# Set interactively on first launch, or explicitly here:
# password = "your chosen password"
```

Use `ui.password` as the existing shared login/bearer credential, preserving the current single-credential authentication model. Do not introduce accounts or another authentication service. Store files containing it with mode `0600`, preserve unrelated TOML/comments when first-run setup writes them, and never pass the password in process arguments, logs, URLs, audit entries, or worker environments. Do not include it in run snapshots. Keep current session signing, constant-time credential comparison, expiry, rotation invalidation, same-origin checks, and MCP header-only authentication.

The global settings editor exposes password as a write-only replace field plus a `password_set` boolean. Its Advanced TOML representation omits `ui.password`; omitting that field preserves the stored value. Reject empty replacement passwords. Use a dedicated optional password-update field in the save request rather than a magic masked string. Read/save/validate responses and validation errors must not reveal the password, including when it was supplied in candidate TOML. Explicit manual file editing remains supported.

Launcher host/port flags override `[ui]` for that process. No per-project or launcher alternate workflow config option is added. The new command does not depend on `AFLOW_APP_CONFIG_DIR`, `AFLOW_EXECUTABLE`, `AFLOW_ENVIRONMENT_FILE`, `AFLOW_RELEASE_IDENTITY`, or manually clearing `VIRTUAL_ENV`. Resolve the installed executable and release identity from the installation executing the command, and prepare private runtime files automatically. An unrelated active virtual environment must not select a different AFlow installation.

Existing low-level `aflow-app-server` deployment configuration may remain as a documented compatibility entry point during this change; it must use the same global workflow resolver and editor semantics. Explicit legacy deployment paths are not inputs required by `aflow ui`. Do not silently import alternate config directories or rewrite project-local files. If a legacy UI config/registry exists elsewhere, show a concise migration instruction; new-command state lives in the canonical global directory. Preserve old project-local config bytes and ignore them for new runs. Existing run ownership and history must not be discarded.
...
Implement the UI adapter behind the existing UnitManager protocol, with durable identity/exit records beneath the existing per-run durable directory. Each launched run has a small detached worker wrapper that owns its workflow subprocess group, writes bounded startup/error/exit evidence, and outlives the UI. Use the installed AFlow executable for the wrapper and worker; do not start another web server or controller per project. The wrapper must record failures before the normal controller starts, as well as normal exits.

Persist the run ID, intended unit, invocation nonce, process birth identity, PID/group identity, and terminal result with versioned atomic writes. Reuse the portable birth-identity approach in daemon_cli.py; require matching invocation and run/root identity before observing/adopting/signalling a process. Do not rely on `/proc` or PID existence alone on macOS. Hold an exclusive per-run launch claim across preparation so concurrent starts/restarts cannot launch duplicate workers. Keep the run metadata as the source of truth, not process-table name matching.
...
1. `aflow/config.py`: `_config_path`, `load_workflow_config`, bootstrap helpers, and project_configuration_state. The default is global, but explicit UI paths bypass it; allowed top-level tables currently exclude `ui`.
...
3. `aflow/daemon_cli.py`: executable/release/environment defaults and PID birth identity offer reusable primitives; current local daemon stop drains children and is not the requested UI lifecycle.
...
6. `apps/aflow_app/server/src/aflow_app_server/{config,project_service,project_config_service,control_plane_service,guided_config,main,browser_session}.py`: readiness and daemon resolution hardcode project `.aflow/config`; config save blocks active/resumable runs and reloads one project; assets resolve relative to the checkout; browser cookies are always Secure.
...
2. **Add the global contract.** Introduce typed UI settings in config.py and a shared resolver used by CLI, readiness, web config, and daemon routing. Implement lossless first-run settings writes and password redaction. Make UI runtime inputs automatic. Test global-only registration with conflicting old local configs preserved.
...
uv run pytest -q tests/test_config.py tests/test_cli.py tests/test_daemon_cli.py tests/test_control_plane_resume.py tests/test_run_state.py tests/test_resume_relocation.py tests/test_systemd_units.py
...
7. **HTTP browser use:** from a separate LAN/Tailscale browser when available, log in at the server's actual HTTP address, reload, edit a plan/global settings, start a run, observe events, and log out. Wrong passwords and cross-origin writes fail. Password rotation invalidates old sessions; no secret appears in API config responses, logs, snapshots, URL, or browser storage. Automated tests use a non-localhost HTTP hostname to catch Secure-cookie regressions; record any unavailable physical second-device check explicitly.
