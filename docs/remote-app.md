...
- Typed fields only: plan, workflow, team, start step, max turns, and optional extra instructions (at most 8 non-empty strings, each at most 512 characters and 4096 characters total; NUL characters are not allowed). Invalid semantic instruction input returns a stable `invalid_extra_instructions` error without allocating a run; submitted instruction text is never echoed. There is no raw argv or shell input anywhere.
