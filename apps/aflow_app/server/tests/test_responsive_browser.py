...
def test_responsive_team_family_journey(
    ...
    with live_server() as url, sync_playwright() as playwright:
        ...
        try:
            ...
            # Guided section changes keep the unfinished wizard owner mounted.
            _select_settings_section(page, "Changelog")
            page.get_by_role("heading", name="Changelog", exact=True).wait_for()
            expect(page.get_by_label("Family display name", exact=True)).to_be_hidden()
            _select_settings_section(page, "General")
            page.get_by_label("Bind host", exact=True).wait_for()
            expect(page.get_by_label("Family display name", exact=True)).to_be_hidden()
            _select_settings_section(page, "Teams")
            wizard.wait_for(state="visible")
            expect(wizard.locator(".team-family-wizard-base-summary")).to_contain_text(family_label)
            expect(wizard.locator(".team-family-wizard-base-summary")).to_contain_text("browser_family")
            expect(wizard.locator(".team-family-wizard-steps .current")).to_contain_text("Stages")
            expect(wizard.get_by_label("Stable ID for stage 1", exact=True)).to_have_value(strong_stage_id)
            expect(wizard.get_by_label(f"Worker for stage {strong_stage_id}", exact=True)).to_have_value(re.compile(r"^codex\.strong(?:\s|$)"))
            wizard.get_by_role("button", name="Back", exact=True).click()
            expect(wizard.get_by_label("Family display name", exact=True)).to_have_value(family_label)
            wizard.get_by_text("Technical details", exact=True).first.click()
            expect(wizard.get_by_label("Stable Base ID", exact=True)).to_have_value("browser_family")
            expect(wizard.get_by_label("Base assignment source", exact=True)).to_have_value("product")
            wizard.get_by_role("button", name="Next: stages", exact=True).click()
            expect(wizard.locator(".team-family-wizard-steps .current")).to_contain_text("Stages")
