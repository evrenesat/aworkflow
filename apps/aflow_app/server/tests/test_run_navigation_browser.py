        ...
        try:
            for theme in ('light', 'dark'):
                for width, height in ((1440, 900), (390, 844)):
                    ...
                    preparation = page.locator('details.launch-preparation-details')
                    expect(preparation).to_be_visible()
                    expect(preparation).not_to_have_attribute('open', '')
                    preparation.locator(':scope > summary').focus()
                    preparation.locator(':scope > summary').press('Enter')
                    expect(preparation).to_have_attribute('open', '')
                    details = preparation.locator('details.launch-preview-details')
...
def test_remaining_journeys_keep_compact_actions_and_drafts_reachable(control_client, monkeypatch, tmp_path):
    ...
    with live_server() as url, sync_playwright() as playwright:
        ...
        try:
            ...
            preparation.locator(':scope > summary').press('Enter')
            expect(preparation).to_have_attribute('open', '')
            launch_details = preparation.locator('details.launch-preview-details')
