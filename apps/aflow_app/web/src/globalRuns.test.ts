...
describe('global runs', () => {
  ...
  it('classifies waiting, starting, failures and unknown liveness honestly', () => {
    for (const status of ['paused', 'running', 'waiting_for_input', 'waiting_for_valid_override', 'awaiting_startup_answer', 'launch_requested']) expect(isOngoing(run('x', status))).toBe(true)
    expect(isOngoing(run('x', 'needs_attention'))).toBe(false)
    expect(isOngoing({ ...run('x', 'manifest_only'), evidence: { startup_state: 'preparing' } })).toBe(true)
    expect(isOngoing({ ...run('x', 'launch_requested'), evidence: { startup_failure: { stage: 'unit_launch' } } })).toBe(false)
    expect(isOngoing({ ...run('x', 'needs_attention'), evidence: { startup_failure: {}, unit_active: true } })).toBe(true)
