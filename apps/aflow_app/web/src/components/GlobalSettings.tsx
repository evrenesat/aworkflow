...
export function GlobalSettings({ onDirtyChange, onSaved, project = null }: { onDirtyChange: (dirty: boolean) => void; onSaved: (saved: ProjectConfig) => void; project?: ProjectInfo | null }) {
  ...
  function repairThresholdText(current: GuidedFormProjection, workflow: string): string {
    ...
    return display.value === null
      ...
      : `Effective: ${display.value} (${managerSourceLabel(display.source)}). Leave blank to inherit. 0 = switch for first reviewer-requested repair; positive values count failed repairs.`
  }
  ...
  async function save() {
    ...
    try {
      if (draft?.default_upgrade_after_repairs != null && !nonnegativeInteger(draft.default_upgrade_after_repairs)) {
        throw new Error('Default repair threshold must be a nonnegative integer')
      }
      for (const [workflow, value] of Object.entries(draft?.workflows ?? {})) {
        if (value.upgrade_after_repairs != null && !nonnegativeInteger(value.upgrade_after_repairs)) {
          throw new Error(`Repair threshold for ${workflow} must be a nonnegative integer`)
        }
      }
  ...
  return <div className="workspace-content global-settings">
    ...
    /></div>{draft && <div className="settings-retained-team-families" hidden={tab !== 'Teams'} aria-hidden={tab !== 'Teams' || undefined}>
      ...
      <details className="settings-disclosure"><summary>Change password</summary>
        <div className="settings-fields"><label>New password (leave empty to keep current)<input className="input" type="password" autoComplete="new-password" value={password} onChange={e => setPassword(e.target.value)} /></label>
        {password && <p>Saving the password ends existing sessions.</p>}</div>
      </details>
      ...
      {tab === 'Agents & Roles' && <>
        ...
        <h3>Global roles</h3>
        <p className="settings-section-lead">Effective global role assignments remain editable here; team-specific inheritance is shown in Teams.</p>
        <div className="settings-effective-list">{Object.entries(draft.roles).map(([role, selector]) => <div className="settings-effective-row" key={role}><strong>{formatMachineChoice(role, Object.keys(draft.roles))}</strong><Combobox label={`Role ${formatMachineChoice(role, Object.keys(draft.roles))}`} visibleLabel="Effective profile" value={selector} options={selectors} onChange={value => change(next => { next.roles[role] = value })} /></div>)}</div>
        <details className="settings-disclosure">
          <summary>Add global role</summary>
