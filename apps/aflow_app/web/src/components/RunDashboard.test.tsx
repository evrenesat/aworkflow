...
async function startRun() {
  const continuation = screen.queryByRole('button', { name: 'Confirm and continue', exact: true })
  if (continuation) {
    fireEvent.click(continuation)
    return
  }
  const review = screen.queryByRole('button', { name: 'Review start…', exact: true })
  if (!review) {
    fireEvent.click(screen.getByRole('button', { name: 'Start run', exact: true }))
    return
  }
...
function startActionButton(): HTMLElement {
  return screen.queryByRole('button', { name: 'Review start…', exact: true })
    ?? screen.queryByRole('button', { name: 'Confirm and continue', exact: true })
...
async function waitForPreflightReady(
  ...
  if (intendedAction === 'start' && validatedButton && !(validatedButton as HTMLButtonElement).disabled) {
    fireEvent.click(validatedButton)
    return screen.findByRole('button', { name: 'Start run', exact: true })
  }
...
describe('RunDashboard', () => {
  ...
  it('leads a failed reviewer run with its recorded step and a bounded failure explanation', async () => {
    const failed = { ...ownedRun, status: 'failed', activity: 'inactive' as const, launch_phase: 'failed',
      current_step: 'review', turns_completed: 4, reason: null }
    vi.mocked(api.listControlPlaneRuns).mockResolvedValue({ runs: [failed], next_cursor: null, schema_version: 1 })
    vi.mocked(api.getControlPlaneRun).mockResolvedValue(failed)
    vi.mocked(api.listRunEvents).mockResolvedValue([
      { sequence: 1, event_type: 'turn_started', data: { turn_number: 4, step_name: 'review', role: 'reviewer' }, schema_version: 1, timestamp: '2026-09-23T14:07:51Z' },
      { sequence: 2, event_type: 'turn_finished', data: { turn_number: 4, step_name: 'review', role: 'reviewer', status: 'failed', summary: 'exit 1' }, schema_version: 1, timestamp: '2026-09-23T14:09:00Z' },
      { sequence: 3, event_type: 'run_failed', data: { turn_number: 4, status: 'failed', message: '# AFlow manager report ## Summary' }, schema_version: 1, timestamp: '2026-09-23T14:09:01Z' },
    ])
    vi.mocked(api.getRunContext).mockResolvedValue(progressContext(observerProgress({
      checkpoint: { index: 2, name: 'Checkpoint 2: Review fidelity' },
      last_finished_turn: { turn_number: 4, step: 'review', status: 'failed', summary: 'exit 1' },
    })))
    renderDashboard()
    expect(await screen.findByText(/Review stopped · CP2/)).toBeDefined()
    expect(screen.getByText(/Review stopped before a decision\. The specific cause/)).toBeDefined()
    expect(screen.queryByText(/No current work — Failed/)).toBeNull()
    expect(screen.queryByText(/Recorded update: exit 1/)).toBeNull()
    expect(screen.queryByText(/# AFlow manager report/)).toBeNull()
    expect(document.querySelectorAll('.run-overview-event')).toHaveLength(2)
  ...
  it('explains a recorded reviewer harness failure before a decision without exposing raw tokens', async () => {
    const failed = { ...ownedRun, status: 'failed', activity: 'inactive' as const, launch_phase: 'failed',
      current_step: 'review_checkpoint', turns_completed: 4, reason: '# AFlow manager report\n## Summary' }
    vi.mocked(api.listControlPlaneRuns).mockResolvedValue({ runs: [failed], next_cursor: null, schema_version: 1 })
    vi.mocked(api.getControlPlaneRun).mockResolvedValue(failed)
    vi.mocked(api.listRunEvents).mockResolvedValue([
      { sequence: 1, event_type: 'turn_finished', data: { turn_number: 4, step_name: 'review_checkpoint', role: 'reviewer', outcome: 'harness-failed', returncode: 1 }, schema_version: 1, timestamp: '2026-09-23T14:09:00Z' },
    ])
    vi.mocked(api.getRunContext).mockResolvedValue(progressContext(observerProgress({
      checkpoint: { index: 2, name: 'Checkpoint 2: Review fidelity' },
      last_finished_turn: { turn_number: 4, step: 'review_checkpoint', status: 'failed', summary: 'harness-failed' },
    })))
    renderDashboard()
    expect(await screen.findByText('The reviewer harness failed before a decision.')).toBeDefined()
    expect(screen.getByText(/Review stopped · CP2/)).toBeDefined()
    const result = document.querySelector('[data-ui-fidelity-anchor="latest-result"]')
    expect(result?.textContent).not.toMatch(/harness-failed|# AFlow manager report|exit 1/)
    fireEvent.click(screen.getByRole('link', { name: 'Open Diagnostics' }))
    expect(screen.getByRole('button', { name: 'Diagnostics' }).getAttribute('aria-expanded')).toBe('true')
  })
  ...
  it('keeps a meaningful short failed-review reason ahead of the harness fallback', async () => {
    ...
    renderDashboard()
    expect(await screen.findByText('Reviewer process lost its connection.')).toBeDefined()
    expect(screen.queryByText('The reviewer harness failed before a decision.')).toBeNull()
  })
  ...
  it('keeps launch review read-only until the final start action', async () => {
    ...
    expect(screen.getByText('Read-only check — no run has been allocated.')).toBeDefined()
    expect(screen.getByText('Ready for the final start action.')).toBeDefined()
    ...
    expect(screen.queryByRole('region', { name: 'Review start', exact: true })).toBeNull()
    expect(api.startControlPlaneRun).not.toHaveBeenCalled()
  })
  ...
  it('invalidates a launch review when its request identity changes', async () => {
    ...
    choose('Run plan', alternatePlan.path)
    await waitFor(() => expect((screen.getByRole('button', { name: 'Start run', exact: true }) as HTMLButtonElement).disabled).toBe(true))
    expect(screen.getByText('Choices changed — close this review and review again.')).toBeDefined()
    expect(api.startControlPlaneRun).not.toHaveBeenCalled()
  ...
  it('blocks a prior ready inspection while committed defaults are refreshed', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Review start…', exact: true }))
    const oldReview = await screen.findByRole('region', { name: 'Review start', exact: true })
    expect(oldReview.textContent).toContain('15 · server default')
    ...
    expect(screen.getByRole('region', { name: 'Review start', exact: true })).toBe(oldReview)
    expect(screen.getByText('Choices changed — close this review and review again.')).toBeDefined()
    expect(api.startControlPlaneRun).not.toHaveBeenCalled()
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Review start…', exact: true }))
    const review = await screen.findByRole('region', { name: 'Review start', exact: true })
    expect(review.textContent).toContain('plans/in-progress/demo.md')
    expect(review.textContent).toContain('Managed')
    expect(review.textContent).toContain('Base')
    expect(review.textContent).toContain('21 · server default')
    expect(review.textContent).toContain('No run is allocated until you choose Start run.')
    const heading = within(review).getByRole('heading', { name: 'Review before starting', exact: true })
    await waitFor(() => expect(document.activeElement).toBe(heading))
  ...
  it('preserves exact successor recovery across run selection and workspace remount', async () => {
    ...
    const destinationStart = await screen.findByRole('button', { name: 'Review start…', exact: true })
  ...
  it('hands off the exact plan path from Configure run and opens New run', async () => {
  ...
  it('shows the server max-turn fallback in the form and final review when the setting is omitted', async () => {
    vi.mocked(api.listControlPlaneRuns).mockResolvedValue({ runs: [], next_cursor: null, schema_version: 1 })
    vi.mocked(api.postGlobalConfigForm).mockResolvedValue({
      ...emptyProjection,
      form: { ...emptyProjection.form, default_workflow: 'managed', max_turns: null },
      server_default_max_turns: 15,
    })
    renderDashboard()
    ...
    choose('Run workflow', 'managed')
    expect(screen.getByText('Server default: 15.')).toBeDefined()
    expect(screen.getByLabelText('Effective choices for this launch').textContent).toContain('15 — server default')
    await waitForPreflightReady()

    await screen.findByRole('region', { name: 'Review start', exact: true })
    expect(screen.getByText('15 · server default')).toBeDefined()
    expect(api.startControlPlaneRun).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: 'Cancel review', exact: true }))
    expect(api.startControlPlaneRun).not.toHaveBeenCalled()
  })

  it('invalidates a reviewed launch when committed defaults change before final start', async () => {
    const changedConfig = {
      ...committedConfig,
      revision: 'b'.repeat(64),
      aflow_toml: '# changed committed aflow\n',
    }
    vi.mocked(api.getGlobalConfig)
      .mockResolvedValueOnce(committedConfig)
      .mockResolvedValueOnce(changedConfig)
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Start run', exact: true }))
    await waitFor(() => expect(screen.queryByRole('region', { name: 'Review start', exact: true })).toBeNull())
    expect(screen.getByText(/Committed launch defaults changed while the review was open/)).toBeDefined()
    expect(api.startControlPlaneRun).not.toHaveBeenCalled()
  })
