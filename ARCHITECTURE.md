...
guidelines](UI_GUIDELINES.md), with the integrated responsive editor-readability
and journey coverage extending the same ownership contract.
...
normalizes required Git Tracking metadata only for pristine fresh review plans.
Only after the normalized plan reloads with the same checkpoint snapshot does
the controller reserve a run ID or persist a launch manifest. A workflow in the
current checkout records its symbolic branch and full `HEAD`; a lifecycle
workflow records its selected execution branch and verified base, deferring
only the base value until an eligible bootstrap produces a commit. Explicit
nonempty identity is not replaced because the checkout differs, while started
plans with missing identity are rejected. The later runtime presence check
remains a defensive invariant against external mutation.
...
2. For a pristine fresh review plan with no live Git Tracking section, insert the exact two controller-owned fields atomically and reload the plan. For an existing pristine section, fill only blank required fields using the current checkout or lifecycle selection. Existing sections are never rebuilt. Started, resumed, recovery, malformed, ambiguous, and no-HEAD/non-bootstrap inputs fail before a new harness or lifecycle mutation; the daemon may retain a typed idempotent startup-failure record after its reservation boundary.
...
before using affected paths. Failure classification, lifecycle move/recovery,
and explicit requeue ownership compare the resolved parent directory of each
plan, run, and recorded original-plan path to the registered repository
directory. They retain the final entry name without resolving it, so a parent
alias is accepted while symlinked final entries remain rejected. Admission
guards classification and explicit requeue against unresolved run claims.
Requeue validates the edited revision, returns its checked source run with a
stable resume replay key, and resumes a recorded recoverable run through
control-plane admission. Successful delivery retains its receipt-backed done
transition; a failed receipt retains its run claim and
...
The dashboard coordinates manual, timer and stream refreshes, preserves selected-run state on partial failure and gives context a single loader with cancellation and stale-response guards. Direct selected-run snapshots and project-access evidence are keyed by exact project/run identity, so accepted readable detail may render while history or configuration is pending without letting an old identity briefly authorize a new one. History owns its coverage separately: pending or failed pages report incomplete loaded counts, and a pinned direct row never establishes an empty or complete history. Configuration-dependent launch and mutation admission remains gated until the dashboard context resolves. Within selected detail, stable run identity and applicable owner actions precede canonical checkpoint evidence, so late evidence expands below existing pointer targets without withholding that evidence. Summary fields are deterministic; raw artifacts are grouped under collapsed details. Settings owns outstanding prompt-deletion recovery across tabs and editor modes. Browser count edits commit only valid integers on blur/Enter.
