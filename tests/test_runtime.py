...
def _manager_stop_decision(reason: str) -> str:
    return json.dumps({
        "schema_version": 1,
        "action": "stop",
        "reason": reason,
        ...
        "stop_report": {
            ...
            "root_cause": "Synthetic unresolved controller evidence.",
            "evidence": ["The manager selected stop."],
            "attempts": "One finalized workflow turn was inspected.",
            "workspace_state": "The synthetic workspace remains available.",
            "next_actions": ["Inspect the durable manager artifacts."],
        },
    })
...
class WorkflowPreflightTests(unittest.TestCase):
    ...
    def test_review_workflow_without_head_or_lifecycle_fails_before_allocation(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_root = Path(tmpdir)
            plan_path = repo_root / 'plan.md'
            _write_plan(plan_path, '# Plan\n\n### [ ] Checkpoint 1: First\n- [ ] step one\n')
            ...
            assert ctx.value.run_dir is None
            assert plan_path.read_bytes() == original_bytes
            assert runner_calls == []
            assert observer.events == []
            assert not (repo_root / '.aflow').exists()

    def test_review_workflow_bootstraps_missing_git_tracking_before_run_reservation(self) -> None:
    ...
    def test_readme_minimal_review_workflow_reaches_first_turn_after_git_tracking_bootstrap(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def runner(argv, **kwargs):
                runner_calls.append(1)
                text = plan_path.read_text(encoding='utf-8')
                assert text.count('## Git Tracking') == 1
                _write_plan(
                    plan_path,
                    text.replace('### [ ] Checkpoint 1', '### [x] Checkpoint 1').replace(
                        '- [ ] step one',
    ...
    def test_review_workflow_fenced_tracking_example_gets_one_live_section(self) -> None:
        from aflow.plan import _live_git_tracking_heading_line_numbers
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def runner(argv, **kwargs):
                text = plan_path.read_text(encoding='utf-8')
                assert len(_live_git_tracking_heading_line_numbers(text)) == 1
                assert text.count('## Git Tracking') == 2
                _write_plan(
                    plan_path,
                    text.replace('### [ ] Checkpoint 1', '### [x] Checkpoint 1').replace(
                        '- [ ] step one',
    ...
    def test_non_review_workflow_does_not_insert_git_tracking(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def runner(argv, **kwargs):
                text = plan_path.read_text(encoding='utf-8')
                assert '## Git Tracking' not in text
                _write_plan(
                    plan_path,
                    text.replace('### [ ] Checkpoint 1', '### [x] Checkpoint 1').replace(
                        '- [ ] step one',
    ...
    def test_started_plan_missing_git_tracking_fails_before_allocation_unchanged(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_root = Path(tmpdir)
            _make_lifecycle_git_repo(repo_root, branch='main')
            plan_path = repo_root / 'plan.md'
            started = '# Plan\n\n### [ ] Checkpoint 1: First\n- [x] started\n- [ ] step one\n'
            _write_plan(plan_path, started)
            observer = CollectingObserver()

            with patch('aflow.control_plane.reserve_run_id') as reserve:
                with pytest.raises(WorkflowError, match='not pristine') as ctx:
                    run_workflow(
                        ControllerConfig(repo_root=repo_root, plan_path=plan_path, max_turns=1),
                        self._make_review_wf_config(),
                        'review_wf',
            ...
            assert ctx.value.run_dir is None
            assert plan_path.read_text(encoding='utf-8') == started
            assert observer.events == []
            assert not (repo_root / '.aflow').exists()

    def test_resume_missing_git_tracking_fails_before_allocation_unchanged(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_root = Path(tmpdir)
            _make_lifecycle_git_repo(repo_root, branch='main')
            plan_path = repo_root / 'plan.md'
            ...
            with patch('aflow.control_plane.reserve_run_id') as reserve:
                with pytest.raises(WorkflowError, match='during resume') as ctx:
                    run_workflow(
                        ControllerConfig(repo_root=repo_root, plan_path=plan_path, max_turns=1),
                        self._make_review_wf_config(),
                        'review_wf',
    ...
    def test_ambiguous_git_tracking_fails_before_allocation_unchanged(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_root = Path(tmpdir)
            _make_lifecycle_git_repo(repo_root, branch='main')
            plan_path = repo_root / 'plan.md'
            ...
            with patch('aflow.control_plane.reserve_run_id') as reserve:
                with pytest.raises(WorkflowError, match='ambiguous') as ctx:
                    run_workflow(
                        ControllerConfig(repo_root=repo_root, plan_path=plan_path, max_turns=1),
                        self._make_review_wf_config(),
                        'review_wf',
    ...
    def test_recovery_plan_missing_git_tracking_fails_before_allocation_unchanged(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_root = Path(tmpdir)
            _make_lifecycle_git_repo(repo_root, branch='main')
            plan_path = repo_root / 'plan.md'
            ...
            startup_retry = RetryContext(
                ...
                snapshot_before=parsed.snapshot,
                active_plan_path=plan_path,
                new_plan_path=plan_path.with_name('retry.md'),
                base_user_prompt='retry',
                parse_error_str='inconsistent checkpoint state',
                attempt=1,
                retry_limit=1,
            )

            with patch('aflow.control_plane.reserve_run_id') as reserve:
                with pytest.raises(WorkflowError, match='during startup recovery') as ctx:
                    run_workflow(
                        ControllerConfig(repo_root=repo_root, plan_path=plan_path, max_turns=1),
                        self._make_review_wf_config(),
                        'review_wf',
    ...
    def test_stale_prepared_snapshot_rejects_started_plan_before_git_tracking_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_root = Path(tmpdir)
            _make_lifecycle_git_repo(repo_root, branch='main')
            plan_path = repo_root / 'plan.md'
            pristine = '# Plan\n\n### [ ] Checkpoint 1: First\n- [ ] step one\n'
            _write_plan(plan_path, pristine)
            prepared_snapshot = load_plan(plan_path)
            started = pristine.replace('- [ ] step one', '- [x] step one')
            _write_plan(plan_path, started)

            with patch('aflow.control_plane.reserve_run_id') as reserve:
                with pytest.raises(WorkflowError, match='changed after preparation') as ctx:
                    run_workflow(
                        ControllerConfig(repo_root=repo_root, plan_path=plan_path, max_turns=1),
                        self._make_review_wf_config(),
                        'review_wf',
...
class WorkflowLifecycleRuntimeTests(unittest.TestCase):
    ...
    def test_empty_repo_lifecycle_bootstrap_fills_verified_head_before_first_turn(self) -> None:
        from aflow.plan import parse_git_tracking_metadata
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            original = (
                '# Plan\n\n```md\n## Git Tracking\n'
                '- Plan Branch: `fenced-example`\n'
                '- Pre-Handoff Base HEAD: `fenced-base`\n```\n\n'
                '### [ ] Checkpoint 1: First\n- [ ] step one\n'
            )
            _write_plan(plan_path, original)
            wf_config = replace(
                _make_branch_only_wf_config(main_branch='main'),
                prompts={'p': "Use 'aflow-review-checkpoint' with {ACTIVE_PLAN_PATH}."},
            )
            call_count = [0]
            bootstrap_heads: list[str] = []
            ...
            def runner(argv, **kwargs):
                ...
                if call_count[0] == 1:
                    ...
                    rc, head, _ = _run_git_in_test(['rev-parse', '--verify', 'HEAD'], cwd=cwd)
                    assert rc == 0
                    bootstrap_heads.append(head)
                    return subprocess.CompletedProcess(argv, 0, 'bootstrap ok', '')
                if call_count[0] == 2:
                    synchronized_text = plan_path.read_text(encoding='utf-8')
                    assert '- Plan Branch: `fenced-example`' in synchronized_text
                    assert '- Pre-Handoff Base HEAD: `fenced-base`' in synchronized_text
                    metadata = parse_git_tracking_metadata(synchronized_text)
                    assert metadata is not None
                    rc, branch, _ = _run_git_in_test(['branch', '--show-current'], cwd=cwd)
                    assert rc == 0
                    assert metadata.plan_branch == branch
                    assert metadata.pre_handoff_base_head == bootstrap_heads[0]
                    completed = plan_path.read_text(encoding='utf-8').replace(
                        '### [ ] Checkpoint 1',
                        '### [x] Checkpoint 1',
                    ).replace('- [ ] step one', '- [x] step one')
                    _write_plan(plan_path, completed)
                    subprocess.run(['git', 'add', 'plan.md'], cwd=str(cwd), check=True, capture_output=True)
                    subprocess.run(
                        ['git', 'commit', '-m', 'complete plan'],
                        cwd=str(cwd),
                        check=True,
                        capture_output=True,
                    )
...
class LifecycleBootstrapTests(unittest.TestCase):
    ...
    def test_manager_clean_end_lite_stop_falls_back_once_to_full_continue(self) -> None:
        from aflow.api import CollectingObserver
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def runner(argv, **kwargs):
                ...
                if model == 'lite':
                    return subprocess.CompletedProcess(
                        argv, 0,
                        _manager_stop_decision(
                            'Lite incorrectly tries to stop a clean END.'
                        ),
                        '',
                    )
                return subprocess.CompletedProcess(argv, 0, json.dumps({
            ...
            assert calls == ['worker', 'lite', 'full']
            lite_dir = result.run_dir / 'manager' / 'decision-001'
            full_dir = result.run_dir / 'manager' / 'decision-002'
            lite_result = json.loads((lite_dir / 'result.json').read_text(encoding='utf-8'))
            full_result = json.loads((full_dir / 'result.json').read_text(encoding='utf-8'))
            lite_context = json.loads((lite_dir / 'context.json').read_text(encoding='utf-8'))
            full_context = json.loads((full_dir / 'context.json').read_text(encoding='utf-8'))
            assert lite_result['level'] == 'lite'
            assert lite_result['trigger'] == 'post_turn'
            ...
            assert full_result['finalized_turn_number'] == 1
            assert full_context['controller_state']['eligible_actions'] == [
                'continue', 'stop',
            ]
            ...
            assert 'failure_reason' not in run_json
            assert not (result.run_dir / 'manager-report.md').exists()
            event_types = [event.event_type for event in observer.events]
            assert event_types.count(ExecutionEventType.RUN_COMPLETED) == 1
            assert event_types.count(ExecutionEventType.RUN_FAILED) == 0
            manager_events = [
                event for event in observer.events
                if event.event_type == ExecutionEventType.MANAGER_DECIDED
            ]
            assert [
                (event.level, event.action, event.report_path)  # type: ignore[attr-defined]
                for event in manager_events
            ] == [
                ("lite", "invalid", None),
                ("full", "continue", None),
            ]
            assert event_types.index(ExecutionEventType.TURN_FINISHED) < event_types.index(
                ExecutionEventType.MANAGER_STARTED
            )
            assert event_types[-1] == ExecutionEventType.RUN_COMPLETED

    def test_manager_invalid_lite_clean_end_then_full_stop_fails_once(self) -> None:
        from aflow.api import CollectingObserver
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def runner(argv, **kwargs):
                ...
                return subprocess.CompletedProcess(
                    argv, 0,
                    _manager_stop_decision(
                        "Full rejects the clean END after invalid Lite output."
                    ),
                    "",
                )
            ...
            event_types = [event.event_type for event in observer.events]
            assert event_types.count(ExecutionEventType.RUN_FAILED) == 1
            assert event_types.count(ExecutionEventType.RUN_COMPLETED) == 0

    def test_manager_direct_full_clean_end_restores_stop_eligibility(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def force_full_context(*args, **kwargs):
                context = actual_build_context(*args, **kwargs)
                context["controller_state"]["semantic_stall_count"] = 2
                return context
            ...
            def runner(argv, **kwargs):
                ...
                return subprocess.CompletedProcess(
                    argv, 0,
                    _manager_stop_decision(
                        "Direct Full rejects the clean END."
                    ),
                    "",
                )
            ...
            with patch(
                ...
                side_effect=force_full_context,
            ):
                with pytest.raises(WorkflowError) as raised:
                    run_workflow(
                        ControllerConfig(
            ...
            assert result["trigger"] == "post_turn"
            assert result["status"] == "accepted"
            assert result["action"] == "stop"
            assert context["controller_state"]["eligible_actions"] == [
                "continue", "stop",
            ]
            assert (run_dir / "manager-report.md").is_file()
    ...
    def test_manager_clean_end_lite_escalation_then_full_stop_preserves_failure_order(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_root = Path(tmpdir)
            ...
            def runner(argv, **kwargs):
                ...
                if model == "lite":
                    return subprocess.CompletedProcess(argv, 0, json.dumps({
                        ...
                        "reason": "Full should inspect the clean END.",
                        "next_step_notes": [],
                        "stop_report": None,
                    }), "")
                return subprocess.CompletedProcess(
                    argv, 0,
                    _manager_stop_decision("Full rejects the clean END."),
                    "",
                )
    ...
    def test_manager_non_end_lite_stop_emits_report_then_stops_banner_before_raising(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def runner(argv, **kwargs):
                ...
                assert "stop" in context["controller_state"]["eligible_actions"]
                return subprocess.CompletedProcess(
                    argv, 0,
                    _manager_stop_decision(
                        "Stop before the proposed non-END review transition."
                    ),
                    "",
                )
