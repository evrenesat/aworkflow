...
def test_manual_destination_installs_default_child_directories(tmp_path: Path) -> None:
    ...
    assert f"Total copy operations: {len(DEFAULT_BUNDLED_SKILL_NAMES)}" in stdout.getvalue()
...
def test_auto_install_codex_copilot_gemini_and_pi_grouped_preview(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert f"Total copy operations: {len(DEFAULT_BUNDLED_SKILL_NAMES)}" in preview
...
def test_include_optional_includes_optional_skills(tmp_path: Path) -> None:
    ...
    assert copied == len(BUNDLED_SKILL_NAMES)
...
def test_build_install_plan_respects_selection(tmp_path: Path) -> None:
    ...
    assert len(default_plan.skills) == len(DEFAULT_BUNDLED_SKILL_NAMES)
    ...
    assert len(optional_plan.skills) == len(BUNDLED_SKILL_NAMES)
...
def test_only_guard_copies_complete_bundled_skill(tmp_path: Path) -> None:
    ...
    assert (guard_dir / "references" / "aflow-defect-issue.md").is_file()
    assert (guard_dir / "references" / "remote-observation.md").is_file()
    assert (guard_dir / "scripts" / "aflow_guard_snapshot.py").is_file()
    assert (guard_dir / "scripts" / "aflow_guard_issue.py").is_file()
    assert not (guard_dir / "scripts" / "aflow_guard_report.py").exists()
    assert not (guard_dir / "references" / "reporting-and-email.md").exists()
