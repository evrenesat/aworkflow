...
- 2026-09-10: Bounded CP10-v02 repair restores scope-less stopped-run reviewer targeting from the predecessor's latest finalized worker result metadata, preserves active-scope and pending-boundary precedence, and adds synthetic resume/ambiguity coverage with active-overlay evidence. Required runtime/state/skill suite: 323 passed, 43 subtests; targeted pending-finalized CLI: 1 passed; source Ruff and diff checks passed. CP10 approval remains unchecked; CP11 untouched. Manager 40 KiB guard and compact 16 KiB summary retained.

- 2026-09-10: Rejected pending CP10 worker worktree against approved CP9 `ff7f388`; worktree fallback, zero intervening implementation commits. Synthetic actual resume reproduces P2: scope-less normally stopped runs omit the pending reviewer target because recovery requires unfinished manager-boundary replay. Required CP10 suite: 321 passed, 43 subtests; focused CLI: 1 passed; diff check passed. CP10 approval remains unchecked, compatible work retained, CP11 untouched. Non-checkpoint CP10 repair overlay: `live-configuration-without-snapshot-gates-20260909-cp10-v02.md`. Manager 40 KiB guard and compact 16 KiB summary retained.
...
### [x] Checkpoint 10: Give checkpoint reviewers the pending implementation target
...
- [x] Implement invariant 13 using the existing awaiting-review scope and finalized worker evidence. Append a compact “Checkpoint under review” section to actual reviewer prompts with the original checkpoint index/name, active overlay when present and worker artifact reference. Do this for both ordinary and resumed review invocations; do not render `{NEXT_CP}` as review target when it names future implementation work.
- [x] For recovered runs without a scope, read existing worker result metadata and the original plan's existing review markers/log to choose the pending target. If no unambiguous target exists, explain the missing information and allow an explicit user target rather than quietly repeating the newest commit. Preserve incomplete/non-checkpoint plan behavior outside checkpoint review.
- [x] Update bundled reviewer target selection: explicit user target first; otherwise supplied pending target; otherwise next implemented-but-unapproved checkpoint from existing ledger/evidence. A `cpN vNN` commit already recorded approved is a base for pending dirty work, not the default review target. Retain commit-boundary review when that commit actually represents the pending checkpoint. Do not edit installed user skill copies as part of source implementation.
- [x] Test CP6 approved/CP7 dirty/first unchecked CP8; a pending repair overlay; normal worker→review scope; and stopped-run resume without active scope. Assert captured review prompt selects CP7 and its original heading and does not select CP6 or CP8. Keep review rejection/next-step behavior unchanged.
...
## CP13 delivery acceptance amendment — 2026-09-10

The already-published9d11b37 release has a green packaging/full-browser CI gate, which must remain green for this live-configuration release. Inspection found scripts/smoke_ui.py still requires `.aflow/runs/<run>/config/snapshot.json` after UI restart and claims a frozen snapshot survived. That obsolete assertion contradicts this plan. CP13 explicitly includes scripts/smoke_ui.py and directly related packaging smoke tests: replace the frozen snapshot requirement with validation of the new durable `live_config_path`/current-source provenance actually persisted by the completed run, pointing to the smoke's own temporary configured source, while preserving run identity/terminal status/history/authenticated same-port restart assertions. Do not merely remove all configuration evidence checks or restore snapshots for the test. Extend the stale-contract search to scripts and CI definitions.

Build the final wheel and run `python3 scripts/smoke_ui.py --verify-wheel <exact-wheel>` and `python3 scripts/smoke_ui.py --wheel <exact-wheel> --browser --playwright-python /root/code/agent_flow/.venv/bin/python` against the disposable synthetic harness runtime; full flow, not login-only. Diagnose any directly related live-source compatibility failure within CP13. Published delivery fixes c6b6437,b9e8632,85abd0e,9d11b37 live on origin/main but not yet in this execution branch; At the START of CP13, after CP12 approval leaves the execution checkout clean, integrate that exact already-published main revision with `git merge --no-ff --no-commit 9d11b373f8820209fa3c827002840267696caf6c` before final build verification. This accepted-main integration is explicitly in CP13 scope, including its existing root AGENTS publication grant; do not manually rewrite that file. Preserve both histories, live-config semantics, and accepted publication/CI/listener fixes. Resolve only actual integration conflicts without discarding either side. Leave MERGE_HEAD and the final checkpoint commit to the reviewer, who is authorized to create the two-parent CP13 approval commit. Review the new CP13 work and integration outcomes, not an unrelated whole-plan redo of the already-published features. If the checkpoint starts with unexpected unowned changes, do not merge over them. This avoids falsely failing CP13 packaging on the known listener/CI defects already fixed on main. Coordinator still owns final publication and exact-SHA CI/live verification. Do not reimplement or undo their matrix/browser prerequisites/listener fixes. Standing owner authorization now permits completed reviewed AFlow publication to origin/main and existing CI deployment; coordinator owns publishing and live proof for this old pinned controller. No new global-config migration is needed: it was already applied for9d11b37.
